...
export class OwnerPortal {
  ...
  async cancelReservation(session, id, body) {
    ...
    return this.reservationMutation(session, id, async (unit, key) => {
      ...
      try {
        const current = await this.bookings(session, "current", { unit });
        const unpaid = await this.bookings(session, "unpaid", { unit });
        if ([...current, ...unpaid].some((booking) => booking.id === id))
          throw new AppError(
