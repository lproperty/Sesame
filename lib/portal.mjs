...
import { AppError } from "./upstream.mjs";
import {
  QUOTE_TTL_MS,
  bookingWindow,
  validateDate,
...
export class OwnerPortal {
  ...
  scope(session, unit = session.unit) {
    return digest([session.user.id, unit?.unitId]);
  }
  ...
  async preview(session, body) {
    ...
    const quote = {
      ...
      termsHash: digest([facility.regulations, facility.notice]),
      attempted: false,
      result: null,
    };
  ...
  async submitQuote(session, quote, action) {
    ...
    if (
      ...
      digest([facility.regulations, facility.notice]) !== quote.termsHash
    ) {
      throw new AppError(
