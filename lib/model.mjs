...
export function normalizeSlot(slot, facility, now = Date.now()) {
  ...
  const endAt = Date.parse(`${date}T${endTime}:00+08:00`);
  const validTime =
  ...
  else if (endAt <= now) reason = "Session has ended";
  ...
  return {
    ...
    enabled: !reason,
    inProgress: !reason && startAt <= now,
    ended: Number.isFinite(endAt) && endAt <= now,
    reason,
    details: {
      capacity,
      remainingCapacity,
...
export function normalizeBooking(record, tab) {
  return {
    ...
    tab,
    orderTime: estateTimestamp(bookingTimestamp(record.orderTime)),
    createdAt:
      estateTimestamp(record.gmtCreate) ||
      estateTimestamp(bookingTimestamp(record.orderTime)),
    updatedAt: estateTimestamp(record.gmtModified),
  };
}

export function isFreeBooking(booking) {
  return booking?.price === 0 && booking?.amount === 0;
}

export function freeTennisCancellationCandidate(booking, now = Date.now()) {
  if (
